...
def create_bco_dmo_json(
    index, dataset_id, titles, investigators, dataset_soup, lon_lat_list
):
...
def get_geojson(dataset_id, processing_log):
    ...
    url = f"https://erddap.bco-dmo.org/erddap/tabledap/bcodmo_dataset_{str(dataset_id)}.geoJson"
...
def get_bco_dmo_json(
    index,
    page,
    number_of_datasets_per_page,
    dataset_id,
    titles,
    investigators,
    processing_log,
    ...
    dataset_soup = get_dataset_soup(
        dataset_id, processing_log, page, number_of_datasets_per_page
    )
    ...
    dataset_df = ctd.put_dataset_into_dataframe(
        dataset_id, processing_log, page, index, number_of_datasets_per_page
    )
    ...
    (
        ...
        latitude_col_index,
        longitude_col_index,
    ) = ctd.check_column_names_for_ctd(column_names)

    if is_ctd:

        lon_lat_list = get_lon_lat_list(
            dataset_df, pressure_col_index, latitude_col_index, longitude_col_index
        )
    ...
    bco_dmo_json = create_bco_dmo_json(
        index, dataset_id, titles, investigators, dataset_soup, lon_lat_list
    )
...
def main():
    ...
    for index, (page, dataset_id) in enumerate(page_dataset_ids):
        ...
        print(
            f"Processing dataset id {dataset_id} on page {page + 1}: number {index+1} out of {len(page_dataset_ids)}"
        )

        bco_dmo_json = get_bco_dmo_json(
            index,
            page,
            number_of_datasets_per_page,
            dataset_id,
